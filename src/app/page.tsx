  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          paddingTop: "calc(max(16px, env(safe-area-inset-top)) + 24px)",
          paddingBottom:
            activeTab === "home"
      ...
      >
        {activeTab === "home" ? (
          <>
            {/* 상단 텍스트 */}
            <div className="w-full max-w-md text-center px-5 mt-2 mx-auto">
                ...
                >
                  ...
                  {(item.dong || item.areaSqm || item.floor !== undefined) && (
                    <p className="text-[#868E96] text-[13px] font-normal leading-[18px] tracking-[-0.26px]">
                      {[item.dong, item.areaSqm ? `${item.areaSqm}㎡` : "", item.floor !== undefined ? `${item.floor}층` : ""]
                        .filter(Boolean)
                        .join(" · ")}
                    </p>
                  )}
      ...
      {activeTab === "home" && (
        <div className="fixed bottom-[56px] left-0 right-0 flex justify-center z-50">
          <div
      ...
      <div className="fixed bottom-0 left-0 right-0 flex justify-center z-50 border-t border-[#E9ECEF] bg-white">
        <div
          className="flex w-full max-w-md px-5 pt-2 pb-[calc(8px+env(safe-area-inset-bottom))] gap-2 items-center"
        >
          <button
            onClick={() => setActiveTab("home")}
            className={`flex-1 h-11 justify-center items-center gap-2.5 flex rounded-[300px] text-sm font-semibold ${
              activeTab === "home"
                ? "bg-[#000000] text-white"
                : "border border-[#ADB5BD] text-grey-100"
            }`}
          >
            홈
          </button>
