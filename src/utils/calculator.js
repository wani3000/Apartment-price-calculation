 ...
 * @returns 월 이자액
 */
function calculateMonthlyInterestOnly(principal, rate) {
  // 월이자율 변환
  const monthlyRate = rate / 100 / 12;
 ...
 * @returns 대출 한도 (원)
 */
function calculateLoanLimit(monthlyDSR, rate, years) {
  // 월이자율 변환
  const monthlyRate = rate / 100 / 12;
  // 총 납입 횟수
  const payments = years * 12;
 ...
 */
function calculateMaxPurchaseForLiving(
  annualIncome,
  assets,
  dsrRatio,
  loanRate = 3.5,
 ...
 * @param {number} assets 보유자산 (원)
 * @param {number} dsrRatio DSR 비율 (%) - 40% 또는 50%
 * @param {boolean} isCapitalArea 수도권 여부 (서울·경기·인천) - 기본값 true
 * @param {number} loanRate 기본 대출 이자율 (%) - 기본값 3.5%
 * @param {number} loanYears 대출 기간 (년) - 기본값 40
 * @param {number} ltv LTV 비율 (%) - 기본값 70%
 * @returns {{ maxPropertyPrice: number, mortgageLimit: number, creditLoan: number, stressRate: number, effectiveRate: number }}
 */
function calculateMaxPurchaseForLivingWithStressDSR(
  annualIncome,
  assets,
  dsrRatio,
  isCapitalArea = true,
 ...
 */
function calculateMaxPurchaseForInvestment(
  annualIncome,
  assets,
  jeonseRatio = 60
) {
