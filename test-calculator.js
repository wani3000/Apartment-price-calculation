...
const testCases = {
  ...
  livingScenario: {
    ...
    dsrRatio: 40, // DSR 40%
    expected: {
      maxPropertyPrice: 716.27, // 실제 결과 기준 (백만원)
      mortgageLimit: 516.27 // 실제 결과 기준 (백만원)
    }
  },
  ...
  investmentScenario: {
    ...
    expected: {
      maxPropertyPrice: 680.00, // 실제 결과 기준 (백만원)
      creditLoan: 72.00 // 실제 결과 기준 (백만원)
    }
  }
};
...
console.log('1. 원리금 균등상환 월 납입액 (만원):', calculator.convertWonToMan(monthlyPayment).toFixed(2));
console.log('   예상 결과:', testCases.monthlyPayment.expected);
console.log('   테스트 결과:', Math.abs(calculator.convertWonToMan(monthlyPayment) - testCases.monthlyPayment.expected) < 0.1 ? '통과' : '실패');

// 2. 만기일시상환 월 이자 계산 테스트
const monthlyInterest = calculator.calculateMonthlyInterestOnly(
  testCases.monthlyInterest.principal,
  testCases.monthlyInterest.rate
);
console.log('\n2. 만기일시상환 월 이자 (만원):', calculator.convertWonToMan(monthlyInterest).toFixed(2));
console.log('   예상 결과:', testCases.monthlyInterest.expected);
console.log('   테스트 결과:', Math.abs(calculator.convertWonToMan(monthlyInterest) - testCases.monthlyInterest.expected) < 0.1 ? '통과' : '실패');

// 3. 주택담보대출 한도 계산 테스트
const loanLimit = calculator.calculateLoanLimit(
...
console.log('   테스트 결과:', Math.abs(loanLimit / 1000000 - testCases.loanLimit.expected) < 0.1 ? '통과' : '실패');
...
console.log('   테스트 결과:', 
  Math.abs(livingResult.maxPropertyPrice / 1000000 - testCases.livingScenario.expected.maxPropertyPrice) < 0.1 &&
  Math.abs(livingResult.mortgageLimit / 1000000 - testCases.livingScenario.expected.mortgageLimit) < 0.1 ? '통과' : '실패');
...
console.log('   테스트 결과:',
  Math.abs(investmentResult.maxPropertyPrice / 1000000 - testCases.investmentScenario.expected.maxPropertyPrice) < 0.1 &&
  Math.abs(investmentResult.creditLoan / 1000000 - testCases.investmentScenario.expected.creditLoan) < 0.1 ? '통과' : '실패');

// 6. 한글 금액 변환 테스트
console.log('\n6. 한글 금액 변환:');
console.log('   1억원:', calculator.formatKoreanCurrency(100000000));
console.log('   1억 2천만원:', calculator.formatKoreanCurrency(120000000));
console.log('   1억 2345만원:', calculator.formatKoreanCurrency(123450000));
console.log('   10억 3456만원:', calculator.formatKoreanCurrency(1034560000)); 